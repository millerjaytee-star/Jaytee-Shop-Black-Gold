-- Privacy-preserving Ask Stabilis usage and cost observability.
-- Raw operator questions are not retained. Estimated cost uses published model
-- token rates captured by the application as a cost_basis string; provider
-- billing/Netlify AI Gateway remains the billing source of truth.
...
begin
  ...
  insert into stabilis.intelligence_queries(
    ...
    p_estimated_cost_usd,left(p_cost_basis,200)
...
comment on function public.stabilis_log_intelligence_query_v2(uuid,jsonb,text,text,text,text,text,jsonb,integer,text,text,integer,text,text,text,text,integer,integer,integer,numeric,text)
is 'Tenant-authorized Ask Stabilis audit log with query fingerprint, model/prompt versions, token usage, latency, outcome and approximate model cost.';
